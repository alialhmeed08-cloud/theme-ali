...
// ✅ PhoneX — theme toggle + scroll فقط (countdown ينشغل في home.js عشان ما يكرر)
...
class App extends AppHelpers {
  ...
  loadTheApp() {
    ...
    // ✅ PhoneX UI features (global — تشتغل في كل صفحة)
  ...
  log(message) {
    salla.log(`ThemeApp(PhoneX)::${message}`);
    return this;
  }
    ...
    setTimeout(() => {
      app.all('.root-level.has-children', item => {
        ...
        app.on('mouseover', item, () => {
          item.querySelectorAll('.sub-menu').forEach((submenu, idx) => {
            if (idx === 0) return;
  ...
  initiateNotifier() {
    salla.notify.setNotifier(function (message, type, data) {
      if (window.enable_add_product_toast && data?.data?.googleTags?.event === "addToCart") return;
      if (typeof message == 'object') return Swal.fire(message).then(type);
  ...
  initiateStickyMenu() {
    let header = this.element('#phonenx-nav') || this.element('#mainnav'),
      height = (this.element('#phonenx-nav') || this.element('#mainnav .inner'))?.clientHeight;
    if (!header) return;
  ...
  setHeaderHeight() {
    let el = this.element('#phonenx-nav') || this.element('#mainnav .inner');
    let header = this.element('#phonenx-nav') || this.element('#mainnav');
    if (!el || !header) return;
    header.style.height = el.clientHeight + 'px';
  }
  ...
  toggleModal(id, isOpen) {
    ...
    if (!isOpen) setTimeout(() => this.addClass(id, 'hidden'), 350);
  }
  ...
  initiateCollapse() {
    document.querySelectorAll('.btn--collapse').forEach((trigger) => {
      ...
      trigger.addEventListener('click', () => {
        this.toggleElementClassIf([content, trigger], 'is-closed', 'is-opened', () => state.isOpen);
...
// ✅ salla.onReady هو الـ entry point الصحيح مع سلة SDK
